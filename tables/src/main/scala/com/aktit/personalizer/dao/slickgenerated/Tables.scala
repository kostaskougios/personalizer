...
trait Tables {
  ...
  implicit def GetResultDirectoryStatusRow(
    implicit e0: GR[Int],
    e1: GR[String],
    e2: GR[Short]
  ): GR[DirectoryStatusRow] = GR { prs =>
    import prs._
  ...
  class DirectoryStatus(_tableTag: Tag)
      extends profile.api.Table[DirectoryStatusRow](
        _tableTag,
        "directory_status"
      ) {
    def * =
      (id, path, status) <> (DirectoryStatusRow.tupled, DirectoryStatusRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      ((Rep.Some(id), Rep.Some(path), Rep.Some(status))).shaped.<>(
        { r =>
          import r._;
          _1.map(_ => DirectoryStatusRow.tupled((_1.get, _2.get, _3.get)))
        },
        (_: Any) =>
