...
lazy val model = project
  .settings(commonSettings: _*)
  .settings(libraryDependencies ++= {
    Seq(Libraries.ScalaTest, Libraries.Apache.Lang3, Libraries.Apache.CommonsIO)
  })
...
lazy val producers = project
  ...
  .dependsOn(
    ...
    avro % "test->test;compile->compile"
  )

lazy val kafkaProducers = project
  .settings(commonSettings: _*)
  .settings(libraryDependencies ++= {
    Seq(
      Libraries.ScalaTest,
...
lazy val kafkaConsumers = project
  .settings(commonSettings: _*)
  .settings(libraryDependencies ++= {
    Seq(
      Libraries.ScalaTest,
...
lazy val tables =
  project.settings(
    ...
    libraryDependencies ++= {
      import Deps._
      Seq(Slick.Slick, Slick.CodeGen, PostGreSql.Driver)
    }
  )
...
lazy val datacenter = project
  ...
  .dependsOn(
    ...
    model % "test->test;compile->compile",
    avro,
    dao % "test->test;compile->compile"
  )

lazy val exampleSocialNetwork =
  project
    .settings(commonSettings: _*)
    .settings(libraryDependencies ++= {
      Seq(
...
lazy val exampleSocialNetworkDatacenter = project
  .settings(commonSettings: _*)
  .settings(libraryDependencies ++= {
    Seq(
      Libraries.ScalaTest,
